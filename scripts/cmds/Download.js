...
async function download({ videoUrl, message, event }) {
  try {
const addr = (await axios.get(`https://raw.githubusercontent.com/Tanvir0999/stuffs/refs/heads/main/raw/addresses.json`)).data.raw_dl;
    const batman = (await axios.get(`${addr}/scrape/download?url=${videoUrl}`)).data;
    let success = false;
